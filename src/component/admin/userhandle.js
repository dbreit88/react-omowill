...
function UserHandle() {
    ...
    useEffect(() => {
        let isMounted = true;
        axios.post(process.env.REACT_APP_API_URL + '/getAllUser', {})
            .then(response => {
                if (isMounted) {
    ...
    const filteredUsers = usersInfo ? usersInfo.filter(user =>
